 ...
 */
function alphorn_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'alphorn_form_submit';
}


function alphorn_form_submit($form, &$form_state) {
  // Import database alphorn file.
  $alphorn_file = dirname(__FILE__) . '/alphorn.sql';
  $success = import_alphorn($alphorn_file);
...
/**
 * Imports a database alphorn file.
 *
 * @see demo_reset().
 */
function import_alphorn($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open alphorn file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
